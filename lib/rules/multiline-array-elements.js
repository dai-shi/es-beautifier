const {
  fixingBlockStatements,
  fixingObjectProperties,
  fixingArrayExpressions,
...
const create = (context) => {
  ...
  const enterArrayExpression = (node) => {
    ...
    while (parent && parent.loc.start.line === node.loc.start.line) {
      if (fixingArrayExpressions.indexOf(parent) >= 0 ||
          fixingObjectProperties.indexOf(parent) >= 0 ||
          fixingBlockStatements.indexOf(parent) >= 0) {
    ...
    if (fixed) fixingArrayExpressions.push(node);
  };
