...
const create = (context) => {
  ...
  const enterSwitchCase = (node) => {
    if (allowSingleLine &&
      ...
      if (tokenAfterTest.type === 'Punctuator' && node.consequent.length) {
        if (tokenAfterTest.range[1] === node.consequent[0].range[0]) {
          context.report({
            node,
            message: 'Test in a switch case shoule follow a space.',
            loc: tokenAfterTest.loc.end,
            fix: fixer => fixer.insertTextAfter(tokenAfterTest, ' '),
          });
        }
      }
      return;
